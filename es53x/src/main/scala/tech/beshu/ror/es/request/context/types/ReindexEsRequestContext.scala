...
class ReindexEsRequestContext private(actionRequest: ActionRequest,
                                      esContext: EsContext,
  ...
  private def modifyIndicesOf(actionRequest: ActionRequest, indices: NonEmptyList[IndexName]) = {
    Try {
...
object ReindexEsRequestContext {
  def from(actionRequest: ActionRequest,
           esContext: EsContext,
           aclContext: AccessControlStaticContext,
           clusterService: RorClusterService,
           threadPool: ThreadPool): Option[ReindexEsRequestContext] = {
    if (actionRequest.getClass.getSimpleName.startsWith("ReindexRequest")) {
      Some(new ReindexEsRequestContext(actionRequest, esContext, aclContext, clusterService, threadPool))
    } else {
      None
    }
  }
