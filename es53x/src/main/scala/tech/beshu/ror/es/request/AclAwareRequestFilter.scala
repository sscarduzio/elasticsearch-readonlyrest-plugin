...
class AclAwareRequestFilter(clusterService: RorClusterService,
  ...
  private def handleEsRestApiRequest(regularRequestHandler: RegularRequestHandler,
    ...
    esContext.actionRequest match {
      ...
      // rest
      case _ =>
        handleReindexRequest(regularRequestHandler, esContext, aclContext) orElse
  ...
  private def handleReindexRequest(regularRequestHandler: RegularRequestHandler,
                                   esContext: EsContext,
                                   aclContext: AccessControlStaticContext) = {
    ReindexEsRequestContext
