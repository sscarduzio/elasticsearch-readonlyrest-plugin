...
public abstract class RuleActionListener<T extends Rule> {
   ...
   * @return should continue to process the handlers' pipeline
   */
  public boolean onFailure(Rule rule, BlockExitResult result, RequestContext rc, ActionRequest ar, Throwable t) {
    return onFailure(result, rc, ar, t, extractFrom(rule));
  }

  protected abstract boolean onFailure(BlockExitResult result, RequestContext rc, ActionRequest ar, Throwable e, T rule);
