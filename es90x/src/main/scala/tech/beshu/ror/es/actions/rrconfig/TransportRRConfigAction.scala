...
class TransportRRConfigAction(actionName: String,
  ...
  extends TransportNodesAction[RRConfigsRequest, RRConfigsResponse, RRConfigRequest, RRConfig, Void](
    actionName,
    clusterService,
    transportService,
    actionFilters,
  ...
  override def newNodeRequest(request: RRConfigsRequest): RRConfigRequest =
   ...
   private def loadConfig() = doPrivileged {
    RawRorConfigLoadingAction
      .load(EsEnv(env.configDir(), env.modulesDir()), indexContentProvider)
