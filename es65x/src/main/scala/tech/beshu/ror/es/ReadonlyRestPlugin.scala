      ...
      .map(ssl =>
        "ror_ssl_internode" -> new Supplier[Transport] {
          override def get(): Transport = doPrivileged {
            new SSLNetty4InternodeServerTransport(settings, threadPool, networkService, bigArrays, namedWriteableRegistry, circuitBreakerService, ssl, rorEsConfig.fipsConfig.isSslFipsCompliant)
          }
        }
      )
      .toMap
