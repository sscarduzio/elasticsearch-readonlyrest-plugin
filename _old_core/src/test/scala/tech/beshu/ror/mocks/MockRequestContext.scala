...
final case class MockRequestContext[T <: Operation](override val timestamp: Instant = Instant.now(),
...
object MockRequestContext {
  def default[T <: Operation]: MockRequestContext[T] = MockRequestContext()
}
