...
class AuditingTool(settings: Settings,
  ...
  private def toAuditResponse[T <: Operation](responseContext: ResponseContext[T]) = {
    responseContext match {
      case allowedBy: ResponseContext.AllowedBy[T] =>
      ...
      case forbidden: ResponseContext.Forbidden[T] =>
        AuditResponseContext.Forbidden(toAuditRequestContext(forbidden.requestContext, None, forbidden.history))
      case requestedIndexNotExist: ResponseContext.RequestedIndexNotExist[T] =>
      ...
      case errored: ResponseContext.Errored[T] =>
