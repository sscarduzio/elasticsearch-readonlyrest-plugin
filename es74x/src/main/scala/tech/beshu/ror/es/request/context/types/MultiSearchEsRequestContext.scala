...
//      } else {
//        val allRequestIndices = sr.indices().asSafeSet.flatMap(IndexName.fromString)
//        val (clusterIndices, localIndices) = allRequestIndices.partition(_.isClusterIndex)
...
//
//        if (remainingAndClusterIndices.isEmpty) {
//          sr.indices(
//            allRequestIndices
//              .map(i => IndexName.randomNonexistentIndex(i.value.value))
//              .map(_.value.value).toList : _*
//          )
//        } else {
