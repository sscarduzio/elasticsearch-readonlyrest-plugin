...
object EsPatchVerifier extends Logging {
  ...
  def verify(settings: Settings): Unit = {
    val result = for {
      ...
      esPatch <- Try(EsPatch.createA(esHome)).toOption
    } yield {
      if (!esPatch.isPatched) {
        throw new IllegalStateException("ES is not patched. See our docs: https://docs.readonlyrest.com/elasticsearch#3.-patch-es")
      }
    }
    result match {
      case Some(_) =>
