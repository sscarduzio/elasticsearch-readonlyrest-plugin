...
trait MultiNodeRestClient {
  ...
  def performRequestAsync(request: Request, responseListener: ResponseListener)(
      using RequestId
  ): Unit
...
final class RoundRobinClient(client: RestClient) extends MultiNodeRestClient {
  ...
  override def performRequestAsync(request: Request, responseListener: ResponseListener)(
      using RequestId
  ): Unit = {
...
final class FailoverClient private (nodeClients: NonEmptyList[NodeClient])
    extends MultiNodeRestClient
  ...
  private def performWithFailover(nodes: NonEmptyList[NodeClient], request: Request, finalListener: ResponseListener)(
      using RequestId
  ): Unit = {
    nodes match {
