...
trait QueryWithModifiableFields[QUERY <: QueryBuilder] {
  ...
  def handleNotAllowedFieldsIn(query: QUERY, notAllowedFields: NonEmptyList[SpecificField]): QUERY
}

object QueryWithModifiableFields {

  def apply[QUERY <: QueryBuilder](
      implicit ev: QueryWithModifiableFields[QUERY]
  ): QueryWithModifiableFields[QUERY] = ev

  def instance[QUERY <: QueryBuilder](
      f: (QUERY, NonEmptyList[SpecificField]) => QUERY
  ): QueryWithModifiableFields[QUERY] = f(_, _)
  ...
  abstract class ModifiableLeafQuery[QUERY <: QueryBuilder: Leaf: QueryFieldsUsage]
      extends QueryWithModifiableFields[QUERY] {

    protected def replace(query: QUERY, notAllowedFields: NonEmptyList[SpecificField]): QUERY

    override def handleNotAllowedFieldsIn(query: QUERY, notAllowedFields: NonEmptyList[SpecificField]): QUERY = {
      QueryFieldsUsage[QUERY].fieldsIn(query) match {
        case UsingFields(usedFields) =>
          usedFields
            .collect { case specificField: SpecificField =>
              specificField
            }
  ...
  object ModifiableLeafQuery {
    ...
    def apply[QUERY <: QueryBuilder](
        implicit ev: ModifiableLeafQuery[QUERY]
    ) = ev

    def instance[QUERY <: QueryBuilder: Leaf: QueryFieldsUsage](f: (QUERY, NonEmptyList[SpecificField]) => QUERY) =
      new ModifiableLeafQuery[QUERY] {
        override protected def replace(query: QUERY, notAllowedFields: NonEmptyList[SpecificField]): QUERY =
          f(query, notAllowedFields)
      }
  ...
  object instances {
    ...
    // term level
    implicit val existsQueryHandler: ModifiableLeafQuery[ExistsQueryBuilder] = ModifiableLeafQuery.instance {
    ...
    implicit val fuzzyQueryHandler: ModifiableLeafQuery[FuzzyQueryBuilder] = ModifiableLeafQuery.instance {
      (query, notAllowedFields) =>
        QueryBuilders
    ...
    implicit val prefixQueryHandler: ModifiableLeafQuery[PrefixQueryBuilder] = ModifiableLeafQuery.instance {
      (query, notAllowedFields) =>
        QueryBuilders
    ...
    implicit val rangeQueryHandler: ModifiableLeafQuery[RangeQueryBuilder] = ModifiableLeafQuery.instance {
      (query, notAllowedFields) =>
        val newQuery = QueryBuilders
    ...
    implicit val regexpQueryHandler: ModifiableLeafQuery[RegexpQueryBuilder] = ModifiableLeafQuery.instance {
      (query, notAllowedFields) =>
        QueryBuilders
    ...
    implicit val termQueryHandler: ModifiableLeafQuery[TermQueryBuilder] = ModifiableLeafQuery.instance {
      (query, notAllowedFields) =>
        QueryBuilders
    ...
    implicit val termsSetQueryHandler: ModifiableLeafQuery[TermsSetQueryBuilder] = ModifiableLeafQuery.instance {
      (query, notAllowedFields) =>
        val newQuery = new TermsSetQueryBuilder(notAllowedFields.head.obfuscate.value, query.getValues)

        Option(query.getMinimumShouldMatchField) match {
          case Some(definedMatchField) => newQuery.setMinimumShouldMatchField(definedMatchField)
          case None                    => newQuery.setMinimumShouldMatchScript(query.getMinimumShouldMatchScript)
        }
    }

    implicit val wildcardQueryHandler: ModifiableLeafQuery[WildcardQueryBuilder] = ModifiableLeafQuery.instance {
      (query, notAllowedFields) =>
        QueryBuilders
    ...
    implicit val matchPhraseQueryHandler: ModifiableLeafQuery[MatchPhraseQueryBuilder] = ModifiableLeafQuery.instance {
      (query, notAllowedFields) =>
        QueryBuilders
          .matchPhraseQuery(notAllowedFields.head.obfuscate.value, query.value())
          .analyzer(query.analyzer())
          .zeroTermsQuery(query.zeroTermsQuery())
          .slop(query.slop())
          .boost(query.boost())
    }

    implicit val matchPhrasePrefixQueryHandler: ModifiableLeafQuery[MatchPhrasePrefixQueryBuilder] =
      ModifiableLeafQuery.instance { (query, notAllowedFields) =>
        QueryBuilders
          .matchPhrasePrefixQuery(notAllowedFields.head.obfuscate.value, query.value())
          .analyzer(on(query).field("analyzer").get[String]())
    ...
    // compound
    implicit val boolQueryHandler: QueryWithModifiableFields[BoolQueryBuilder] = QueryWithModifiableFields.instance {
      (query, notAllowedFields) =>
        final case class ClauseHandler(
            extractor: BoolQueryBuilder => java.util.List[QueryBuilder],
        ...
        val boolQueryWithNewClauses = clauseHandlers
          ...
          .foldLeft(QueryBuilders.boolQuery()) { case (modifiedBoolQuery, (modifiedClauses, clauseCreator)) =>
    ...
    implicit val boostingQueryHandler: QueryWithModifiableFields[BoostingQueryBuilder] =
      QueryWithModifiableFields.instance { (query, notAllowedFields) =>
    ...
    implicit val constantScoreQueryHandler: QueryWithModifiableFields[ConstantScoreQueryBuilder] =
      QueryWithModifiableFields.instance { (query, notAllowedFields) =>
    ...
    implicit val disjuctionMaxQueryHandler: QueryWithModifiableFields[DisMaxQueryBuilder] =
      QueryWithModifiableFields.instance { (query, notAllowedFields) =>
        query
          .innerQueries()
          .asScala
    ...
    private def handleLeafQuery[QUERY <: QueryBuilder: ModifiableLeafQuery](
        leafQuery: QUERY,
    ...
    private def handleCompoundQuery[QUERY <: QueryBuilder: Compound: QueryWithModifiableFields](
        compoundQuery: QUERY,
        notAllowedFields: NonEmptyList[SpecificField]
    ) = {
