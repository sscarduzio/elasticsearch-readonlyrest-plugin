...
private object LoggerMacro {
  ...
  private[slf4j] def deconstructInterpolatedMessage(message: Expr[CharSequence])(using Quotes): (Expr[CharSequence], Seq[Expr[Any]]) = {
  ...
  private[slf4j] def formatArgs(args: Expr[Seq[Any]])(using q: Quotes): Seq[Expr[Object]] = {
    import quotes.reflect.*
    import util.*
