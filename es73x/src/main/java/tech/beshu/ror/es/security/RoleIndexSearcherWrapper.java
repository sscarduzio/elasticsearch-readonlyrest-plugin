...
public class RoleIndexSearcherWrapper implements Function<IndexService, CheckedFunction<DirectoryReader, DirectoryReader, IOException>> {

  private static final Logger logger = LogManager.getLogger(RoleIndexSearcherWrapper.class);

  @Override
  public CheckedFunction<DirectoryReader, DirectoryReader, IOException> apply(IndexService indexShards) {
    ThreadPool threadPool = indexShards.getThreadPool();
    Function<ShardId, QueryShardContext> queryShardContextProvider = shardId -> indexShards.newQueryShardContext(shardId.id(), null, null, null);
    return reader -> {
