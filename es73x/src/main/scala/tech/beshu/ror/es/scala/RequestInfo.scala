  ...
  override lazy val extractIndices: util.Set[String] = {
    val indices = actionRequest match {
      ...
      case ar: PutIndexTemplateRequest =>
        indicesFromPatterns(clusterService, ar.indices.toSet.asJava).asScala
      case ar: DeleteIndexTemplateRequest =>
        getIndicesRelatedToTemplates(clusterService, Sets.newHashSet(ar.name)).asScala
      case ar: GetIndexTemplatesRequest =>
        if(ar.names().isEmpty) Set("*")
        else getIndicesRelatedToTemplates(clusterService, ar.names().toSet.asJava).asScala
      case ar =>
  ...
  override def writeIndices(newIndices: util.Set[String]): Unit = {
    ...
    actionRequest match {
      ...
      case ar: GetIndexTemplatesRequest =>
        ...
        val allowedTemplateNames = findTemplatesOfIndices(clusterService, newIndices).asScala
      ...
      case _ =>
        ...
        val okSetResult = ReflecUtils.setIndices(actionRequest, Sets.newHashSet("index", "indices"), newIndices)
        if (okSetResult) logger.debug(s"REFLECTION: success changing indices: $newIndices correctly set as $extractIndices")
