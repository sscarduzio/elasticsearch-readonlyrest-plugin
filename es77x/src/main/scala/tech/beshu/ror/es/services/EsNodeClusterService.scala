...
import org.elasticsearch.snapshots.{SnapshotId, SnapshotInfo, SnapshotsService}
    ...
    DataStreamName.Full
      ...
      .map { dataStreamName =>
        FullRemoteDataStreamWithAliases(
          ...
          backingIndices =
            dataStream.getIndices.asSafeList.flatMap(index => IndexName.Full.fromString(index)).toCovariantSet
        )
      }
  }
