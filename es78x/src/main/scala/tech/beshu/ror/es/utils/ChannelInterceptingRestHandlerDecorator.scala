...
class ChannelInterceptingRestHandlerDecorator private(val underlying: RestHandler)
  extends RestHandler with RequestIdAwareLogging {
  ...
  override def deprecatedRoutes(): util.List[RestHandler.DeprecatedRoute] = underlying.deprecatedRoutes()

  override def replacedRoutes(): util.List[RestHandler.ReplacedRoute] = underlying.replacedRoutes()
