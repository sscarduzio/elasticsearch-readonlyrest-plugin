...
if [[ $TRAVIS != "true" ]] ||  [[ $ROR_TASK == "integration_es76x" ]]; then
    echo ">>> es76x => Running testcontainers.."
    ./gradlew integration-tests:test '-PesModule=es76x' || ( find . |grep hs_err |xargs cat && exit 1 )
fi

if [[ $TRAVIS != "true" ]] ||  [[ $ROR_TASK == "integration_es76x_scala" ]]; then
    echo ">>> es76x => Running testcontainers.."
    ./gradlew integration-tests-scala:test '-PesModule=es76x' || ( find . |grep hs_err |xargs cat && exit 1 )
fi
