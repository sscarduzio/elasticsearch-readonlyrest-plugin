...
import tech.beshu.ror.tools.core.patches.internal.modifiers.bytecodeJars.authorization.{DummyAuthorizeInAuthorizationService, MockAuthorizationInfoDuringRetrievingUserPrivilegesInAuthorizationService, MockAuthorizationInfoInSecurityContext}
...
private[patches] class Es83xPatch(rorPluginDirectory: RorPluginDirectory, esVersion: SemVer)
  extends TransportNetty4AwareEsPatch(rorPluginDirectory, esVersion,
    ...
    new XPackCoreJarPatchCreator(
      ...
      MockAuthorizationInfoInSecurityContext
    ),
