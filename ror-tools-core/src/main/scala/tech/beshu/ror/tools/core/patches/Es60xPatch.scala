...
import tech.beshu.ror.tools.core.utils.asm.SecurityActionFilterDeactivatorForEs60xAndAbove.deactivateXpackSecurityFilter

import scala.language.postfixOps
import scala.util.Try

private[patches] class Es60xPatch(esDirectory: EsDirectory,
                                  esVersion: SemVer)
  extends EsPatch {

  private val pluginsPath = esDirectory.path / "plugins"
  ...
  private val xpackSecurityJar = s"x-pack-${esVersion.render}.jar"
  private val xpackSecurityJarPath = pluginsPath / "x-pack" / xpackSecurityJar
