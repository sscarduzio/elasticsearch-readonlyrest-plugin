...
/*
  ...
  so that they always return true, causing the SimpleRole to grant access to any requested index action,
  cluster action, and cluster privilege, regardless of the actual role configuration.
*/
private [patches] object ModifySimpleRoleClass extends BytecodeJarModifier {
