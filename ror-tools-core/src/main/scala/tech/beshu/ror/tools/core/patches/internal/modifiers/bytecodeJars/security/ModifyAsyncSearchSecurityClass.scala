 ...
 *
 * This patch overrides `hasClusterPrivilege(...)` so that it always reports success by
 * calling `listener.onResponse(Boolean.TRUE)` and returning immediately, regardless of the
 * requested privilege name.
 *
 * As a result, Async Search cluster-privilege validation is effectively bypassed, preventing
