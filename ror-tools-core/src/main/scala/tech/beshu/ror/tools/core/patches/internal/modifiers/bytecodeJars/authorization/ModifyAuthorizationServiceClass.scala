...
private[patches] class ModifyAuthorizationServiceClass private(esVersion: SemVer)
    ...
    override def visitMethod(access: Int,
      ...
      name match {
        case "authorize" =>
          esVersion match {
            case v if v >= es8180 =>
              new DummyAuthorizationMethodForEsGreaterOrEqual8180(super.visitMethod(access, name, descriptor, signature, exceptions))
            case v if v >= es7160 =>
  ...
  */
  ...
  private class DummyAuthorizationMethodForEsGreaterOrEqual8180(underlying: MethodVisitor)
    extends MethodVisitor(Opcodes.ASM9) {
    ...
    override def visitCode(): Unit = {
      ...
      // -- begin: resolve indices on Replaceable requests --
