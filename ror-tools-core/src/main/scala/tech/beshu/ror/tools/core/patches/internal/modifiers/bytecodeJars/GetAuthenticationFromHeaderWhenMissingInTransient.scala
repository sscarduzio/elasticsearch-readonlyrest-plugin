...
private [patches] object GetAuthenticationFromHeaderWhenMissingInTransient extends BytecodeJarModifier {
  ...
  override def apply(jar: File): Unit = {
    ...
    val modifiedSecurityClass = loadAndProcessFileFromJar(
      jar = jar,
      classFileName = "org/elasticsearch/xpack/core/security/authc/Authentication",
      processFileContent = doGetAuthenticationFromHeaderWhenMissingInTransient
    )
    updateFileInJar(
      jar = jar,
      destinationPathSting = "org/elasticsearch/xpack/core/security/authc/Authentication.class",
