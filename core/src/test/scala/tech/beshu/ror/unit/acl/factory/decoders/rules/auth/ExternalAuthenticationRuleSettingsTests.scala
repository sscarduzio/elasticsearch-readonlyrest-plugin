...
import tech.beshu.ror.accesscontrol.blocks.definitions.{
  BasicAuthHttpExternalAuthenticationService,
...
class ExternalAuthenticationRuleSettingsTests
    extends BaseRuleSettingsDecoderTest[ExternalAuthenticationRule]
    with MockFactory {
      ...
      "no authentication service with given name is defined" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              RulesLevelCreationError(Message("Cannot find external authentication service with name: ext2"))
            )
          }
        )
      }
      ...
      "authentication service endpoint url is malformed" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              DefinitionsLevelCreationError(Message("""Cannot convert value 'http://sth@{user}/test' to url"""))
            )
          }
        )
      }
      ...
      "authentication service TTL value is negative" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              DefinitionsLevelCreationError(Message("Only positive values allowed. Found: -100 seconds"))
            )
          }
        )
      }
