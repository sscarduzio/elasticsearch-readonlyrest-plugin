    ...
    "be able to be loaded from settings" when {
        ...
        "it contains two apps" in {
          assertDecodingSuccess(
            ...
            assertion = rule => {
              ...
              rule.settings.kibanaIndex should be(AlreadyResolved(kibanaIndexName(".kibana")))
              rule.settings.kibanaTemplateIndex should be(None)
              rule.settings.appsToHide should be(
                Set(
      ...
      "'metadata' property is defined" when {
        "its value is a JSON with all possible JSON types" in {
          ...
          val resolvableMetadataJsonRepresentation = metadataJsonRepresentation
            .toResolvable(variableCreator)
          ...
          assertDecodingSuccess(
            ...
            assertion = rule => {
              ...
              rule.settings.appsToHide should be(Set.empty)
              rule.settings.allowedApiPaths should be(Set.empty)
              rule.settings.genericMetadata should be(Some(resolvableMetadataJsonRepresentation))
              rule.settings.rorIndex should be(RorSettingsIndex(IndexName.Full(".readonlyrest")))
            }
          )
        }
        "its value is NULL" in {
          assertDecodingSuccess(
            ...
            assertion = rule => {
              ...
              rule.settings.appsToHide should be(Set.empty)
              rule.settings.allowedApiPaths should be(Set.empty)
              rule.settings.genericMetadata should be(Some(JsonTree.Value(AlreadyResolved(NullValue))))
              rule.settings.rorIndex should be(RorSettingsIndex(IndexName.Full(".readonlyrest")))
            }
          )
        }
        ...
        "its value is a top-level array" in {
          assertDecodingSuccess(
            yaml = """
                     ...
                     |""".stripMargin,
            assertion = rule => {
              rule.settings.genericMetadata should be(
                Some(
                  JsonTree.Array(
                    List(
        ...
        assertDecodingFailure(
          ...
          assertion = errors => {
            ...
            errors.head should be(
              RulesLevelCreationError(MalformedValue.fromString("""kibana:
                                                                  |  index: ".kibana_custom"
                                                                  |  template_index: ".kibana_template"
                                                                  |  hide_apps:
                                                                  |  - "app1"
