...
class LdapAuthorizationRuleSettingsTests extends BaseRuleSettingsDecoderTest[LdapAuthorizationRule] {
        ...
        assertDecodingSuccess(
          ...
          assertion = rule => {
            rule.settings.groupsLogic should be(
              GroupsLogic.AllOf(GroupIds(UniqueNonEmptyList.of(GroupIdLike.from("g*"))))
            )
            rule.settings.groupsLogic
              .asInstanceOf[GroupsLogic.AllOf]
              .permittedGroupIds
              .ids
              .head shouldBe a[GroupIdPattern]
            assertLdapAuthZServiceLayerTypes(rule.settings.ldap, withServerSideGroupsFiltering = false)
          }
        )
      }
        ...
        assertDecodingSuccess(
          ...
          assertion = rule => {
            ...
            assertLdapAuthZServiceLayerTypes(
              rule.settings.ldap,
              withServerSideGroupsFiltering = false,
          ...
          assertion = errors => {
            ...
            errors.head should be(
              RulesLevelCreationError(
                Message(
                  "ldap_authorization rule requires to define 'groups_any_of'/'groups_all_of'/'groups_not_any_of'/'groups_not_all_of' arrays"
                )
              )
            )
          }
        ...
        assertDecodingFailure(
          ...
          assertion = errors => {
            ...
            errors.head should be(
              RulesLevelCreationError(
                Message(
                  "It is not allowed to use groups_not_any_of and groups_not_all_of rule, when LDAP server-side groups filtering is enabled. Consider using a combined rule, which applies two rules at the same time: groups_or/groups_and together with groups_not_any_of/groups_not_all_of."
                )
              )
            )
          }
      ...
      "LDAP service can only handle authentication, not authorization" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              RulesLevelCreationError(Message("Service: ldap1 cannot be used in 'ldap_authorization' rule"))
            )
          }
        )
      }
