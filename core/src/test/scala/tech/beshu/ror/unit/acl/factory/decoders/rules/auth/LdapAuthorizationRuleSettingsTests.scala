        ...
        assertDecodingSuccess(
          yaml =
            s"""
               ...
               |""".stripMargin,
          assertion = rule => {
            rule.settings.groupsLogic should be(GroupsLogic.Or(GroupIds(UniqueNonEmptyList.of(GroupId("group3")))))
        ...
        assertDecodingSuccess(
          yaml =
            s"""
               ...
               |""".stripMargin,
          assertion = rule => {
            rule.settings.groupsLogic should be (GroupsLogic.Or(GroupIds(
    ...
    "not be able to be loaded from config" when {
      ...
      "LDAP server-side groups filtering is enabled, but list of potentially permitted groups is not provided for groups_not_all_of" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(RulesLevelCreationError(Message("It is not allowed to use groups_not_any_of and groups_not_all_of rule, when LDAP server-side groups filtering is enabled. Consider using a combined rule, which merges the groups_not_any_of/groups_not_all_of with groups_or in a single rule.")))
          }
        )
      }
