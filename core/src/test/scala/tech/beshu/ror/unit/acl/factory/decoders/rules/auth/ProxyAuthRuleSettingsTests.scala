      ...
      "no proxy_auth data is defined" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            ...
            errors.head should be(RulesLevelCreationError(MalformedValue.fromString("""proxy_auth: null
                                                                                      |""".stripMargin)))
          }
        )
      }
      ...
      "the 'proxy_auth_configs' section exists, but not contain any element" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              DefinitionsLevelCreationError(Message("proxy_auth_configs declared, but no definition found"))
            )
          }
        )
      }
      "the 'proxy_auth_configs' section contains proxies with the same names" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            ...
            errors.head should be(
              DefinitionsLevelCreationError(
                Message("proxy_auth_configs definitions must have unique identifiers. Duplicates: proxy1")
              )
            )
          }
        )
