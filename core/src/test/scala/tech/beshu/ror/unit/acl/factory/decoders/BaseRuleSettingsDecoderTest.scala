...
abstract class BaseRuleSettingsDecoderTest[T <: Rule : ClassTag] extends WordSpec with BeforeAndAfterAll with Inside {
  ...
  def assertDecodingSuccess(yaml: String,
    ...
    inside(aFactory.createCoreFrom(rorConfigFromUnsafe(yaml), httpClientsFactory, ldapConnectionPoolProvider).runSyncUnsafe()) { case Right(CoreSettings(acl: AccessControlList, _, _)) =>
  ...
  def assertDecodingFailure(yaml: String,
                            ...
                            aFactory: RawRorConfigBasedCoreFactory = factory(),
                            httpClientsFactory: HttpClientsFactory = MockHttpClientsFactory): Unit = {
    inside(aFactory.createCoreFrom(rorConfigFromUnsafe(yaml), httpClientsFactory, ldapConnectionPoolProvider).runSyncUnsafe()) { case Left(error) =>
      assertion(error)
    }
  }
}
