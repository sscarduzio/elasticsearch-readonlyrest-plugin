...
import tech.beshu.ror.accesscontrol.blocks.definitions.HttpExternalGroupsProviderService.Config.*
import tech.beshu.ror.accesscontrol.blocks.definitions.HttpExternalGroupsProviderService.Config.GroupsConfig.{
  GroupIdsConfig,
  GroupNamesConfig
}
...
class ExternalAuthorizationRuleSettingsTests
    extends BaseRuleSettingsDecoderTest[ExternalAuthorizationRule]
    with MockFactory
    with Inside {
        ...
        assertDecodingSuccess(
          ...
          assertion = rule => {
            inside(rule.settings) { case Settings(service, groupsLogic, users) =>
              ...
              service.asInstanceOf[HttpExternalGroupsProviderService].config should be(
                Config(
        ...
        assertDecodingSuccess(
          ...
          assertion = rule => {
            inside(rule.settings) { case Settings(service, groupsLogic, users) =>
              ...
              underlyingService.config should be(
                Config(
                  url = urlFrom("http://localhost:8080/groups"),
      ...
      "no authorization service with given name is declared" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              RulesLevelCreationError(Message("Cannot find user groups provider with name: GroupsService2"))
            )
          }
        )
      }
      "no authorization service is defined" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              DefinitionsLevelCreationError(Message("user_groups_providers declared, but no definition found"))
            )
          }
        )
      }
      ...
      "authorization service endpoint url is malformed" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            ...
            errors.head should be(
              DefinitionsLevelCreationError(
                Message("Cannot convert value 'http://malformed@{user}:8080/groups' to url")
              )
            )
          }
        )
      ...
      "authorization service token passing method is unknown" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            ...
            errors.head should be(
              DefinitionsLevelCreationError(
                Message("Unknown value 'BODY' of 'auth_token_passed_as' attribute. Supported: 'HEADER', 'QUERY_PARAM'")
              )
            )
          }
        )
      ...
      "authorization service group IDs JSON path is malformed" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              DefinitionsLevelCreationError(Message("Cannot compile '$..groups[?.id)].id_malformed' to JSON path"))
            )
          }
        )
      }
      ...
      "authorization service group names JSON path is malformed" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              DefinitionsLevelCreationError(Message("Cannot compile '$..groups[?.name)].name_malformed' to JSON path"))
            )
          }
        )
      }
      ...
      "authorization service TTL value is malformed" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              DefinitionsLevelCreationError(Message("Cannot convert value '\"hundred\"' to duration"))
            )
          }
        )
      }
      "authorization service TTL value is negative" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              DefinitionsLevelCreationError(Message("Only positive values allowed. Found: -120 seconds"))
            )
          }
        )
      }
      "authorization service http method is unsupported" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            ...
            errors.head should be(
              DefinitionsLevelCreationError(
                Message("Unknown value 'DELETE' of 'http_method' attribute. Supported: 'GET', 'POST'")
              )
            )
          }
        )
