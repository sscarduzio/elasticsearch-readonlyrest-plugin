...
class JwtAuthRuleTests extends AnyWordSpec with MockFactory with BlockContextAssertion with WithDummyRequestIdSupport {
      ...
      "group IDs claim path is defined and groups are passed in JWT token claim" in {
        ...
        assertMatchRule(
          ...
          assertBlockContext(blockContext)(
            ...
            currentGroup = Some(
              GroupId("group1")
            ), // RORDEV-1639 - this behavior changed, the `currentGroup` was not added to the context in pseudo-authorization before
            jwt = Some(domain.Jwt.Payload(jwt.defaultClaims()))
          )
        }
      }
      "group names claim is defined and group names are passed in JWT token claim" in {
        ...
        val jwt = Jwt(
          ...
          claims = List(
            "userId" := "user1",
            Claim(
              NonEmptyList.one(ClaimKey("groups")),
              List(
        ...
        "one group does not have a name" in {
          ...
          assertMatchRule(
            configuredJwtDef = AuthJwtDef(
              ...
              userClaim = domain.Jwt.ClaimName(jsonPathFrom("userId")),
              groupsConfig = GroupsConfig(
                domain.Jwt.ClaimName(jsonPathFrom("groups[?(@.id)].id")),
                Some(domain.Jwt.ClaimName(jsonPathFrom("groups[?(@.name)].name")))
              )
            ),
            tokenHeader = bearerHeader(jwt)
          ) { blockContext =>
  ...
  private def assertRule(
    ...
    val authzSettings = JwtAuthorizationRule.Settings(
      AuthorizationJwtDef(
        configuredJwtDef.id,
        configuredJwtDef.authorizationTokenDef,
        configuredJwtDef.checkMethod,
        configuredJwtDef.groupsConfig
      ),
      groupsLogic,
    )
