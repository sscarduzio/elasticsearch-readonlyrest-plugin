...
class GroupsRuleSettingsTests
    ...
    s"A GroupsRule settings test for $simpleSyntaxName" should {
            ...
            "one, full username is used" in {
              assertDecodingSuccess(
                yaml = s"""
                          ...
                          |""".stripMargin,
                assertion = rule => {
                  val resolvedGroupsLogic =
                    rule.settings.permittedGroupsLogic.resolve(currentUserMetadataRequestBlockContextFrom())
                  ...
                  rule.settings.permittedGroupsLogic
                    .asInstanceOf[RuntimeResolvableGroupsLogic.Simple[GroupsLogic]]
                    .groupIds should be(groups)
                  rule.settings.usersDefinitions.length should be(1)
                  inside(rule.settings.usersDefinitions.head) {
                    case UserDef(_, patterns, WithoutGroupsMapping(authRule, localGroups)) =>
            ...
            "one, username with wildcard is used" in {
              assertDecodingSuccess(
                ...
                assertion = rule => {
                  ...
                  rule.settings.permittedGroupsLogic
                    .asInstanceOf[RuntimeResolvableGroupsLogic.Simple[GroupsLogic]]
                    .groupIds should be(permittedGroups)
                  rule.settings.usersDefinitions.length should be(1)
                  inside(rule.settings.usersDefinitions.head) {
                    case UserDef(_, patterns, WithoutGroupsMapping(authRule, localGroups)) =>
                      ...
                      patterns should be(
                        UserIdPatterns(
                          UniqueNonEmptyList.of(
                            User.UserIdPattern(userId("cartman")),
                            User.UserIdPattern(userId("ca*"))
                          )
                        )
                      )
              ...
              assertDecodingSuccess(
                ...
                assertion = rule => {
                  ...
                  inside(rule.settings.usersDefinitions.head) {
                    case UserDef(_, patterns, WithoutGroupsMapping(authRule, localGroups)) =>
                      patterns should be(UserIdPatterns(UniqueNonEmptyList.of(User.UserIdPattern(userId("cartman")))))
                      localGroups should be(
                        UniqueNonEmptyList.of(group("group1", "Group 1"), group("group3", "Group 3"))
                      )
                      authRule shouldBe an[AuthKeyRule]
                      authRule.asInstanceOf[AuthKeyRule].settings should be {
                        BasicAuthenticationRule.Settings(Credentials(userId("cartman"), PlainTextSecret("pass")))
            ...
            "two usernames used" in {
              assertDecodingSuccess(
                ...
                assertion = rule => {
                  ...
                  inside(rule.settings.usersDefinitions.tail.head) {
                    case UserDef(_, patterns, WithoutGroupsMapping(authRule, localGroups)) =>
                      ...
                      authRule.asInstanceOf[TokenAuthenticationRule].settings should be {
                        TokenAuthenticationRule.Settings(
                          userId("morgan"),
                          StaticToken(anyTokenDef, authorizationTokenFrom("Bearer abc123XYZ"))
                        )
                      }
                  }
                }
            ...
            "no variables are used in group ids" in {
              assertDecodingSuccess(
                ...
                assertion = rule => {
                  ...
                  inside(sortedUserDefinitions.head) {
                    case UserDef(_, patterns, WithoutGroupsMapping(authRule, localGroups)) =>
                      patterns should be(UserIdPatterns(UniqueNonEmptyList.of(User.UserIdPattern(userId("cartman")))))
                      localGroups should be(UniqueNonEmptyList.of(group("group1"), group("group3")))
                      authRule shouldBe an[AuthKeyRule]
                  ...
                  inside(sortedUserDefinitions.tail.head) {
                    case UserDef(_, patterns, WithoutGroupsMapping(authRule, localGroups)) =>
                      patterns should be(UserIdPatterns(UniqueNonEmptyList.of(User.UserIdPattern(userId("morgan")))))
                      ...
                      authRule.asInstanceOf[AuthKeySha1Rule].settings should be {
                        BasicAuthenticationRule.Settings(
                          HashedUserAndPassword("d27aaf7fa3c1603948bb29b7339f2559dc02019a")
                        )
                      }
                  }
                }
            ...
            "a variable in used in one group id" in {
              assertDecodingSuccess(
                ...
                assertion = rule => {
                  ...
                  rule.settings.permittedGroupsLogic.usedVariables.head should be(
                    AlreadyResolved(GroupId("group1").nel)
                  )
          ...
          "separate authentication and authorization rules are used for that" in {
            assertDecodingSuccess(
              ...
              assertion = rule => {
                ...
                val sortedUserDefinitions = rule.settings.usersDefinitions
                inside(sortedUserDefinitions.head) {
                  case UserDef(_, patterns, WithGroupsMapping(Auth.SeparateRules(rule1, rule2), groupMappings)) =>
                    patterns should be(UserIdPatterns(UniqueNonEmptyList.of(User.UserIdPattern(userId("cartman")))))
                ...
                inside(sortedUserDefinitions.tail.head) {
                  case UserDef(_, patterns, WithoutGroupsMapping(rule1, localGroups)) =>
                    patterns should be(UserIdPatterns(UniqueNonEmptyList.of(User.UserIdPattern(userId("morgan")))))
          ...
          "single authorization with authentication rule is used for that" in {
            assertDecodingSuccess(
              ...
              assertion = rule => {
                ...
                val sortedUserDefinitions = rule.settings.usersDefinitions
                inside(sortedUserDefinitions.head) {
                  case UserDef(_, patterns, WithGroupsMapping(Auth.SingleRule(rule1), groupMappings)) =>
                    patterns should be(UserIdPatterns(UniqueNonEmptyList.of(User.UserIdPattern(userId("cartman")))))
                    groupMappings should be(GroupMappings.Simple(UniqueNonEmptyList.of(group("group1"))))
        ...
        "users inside users sections must have the same username patterns" in {
          assertDecodingSuccess(
            ...
            assertion = rule => {
              ...
              inside(sortedUserDefinitions.tail.head) {
                case UserDef(_, patterns, WithoutGroupsMapping(r, localGroups)) =>
                  patterns should be(UserIdPatterns(UniqueNonEmptyList.of(User.UserIdPattern(userId("*")))))
                  localGroups should be(UniqueNonEmptyList.of(group("group2"), group("group3")))
                  r shouldBe an[AuthKeyRule]
                  r.asInstanceOf[AuthKeyRule].settings should be {
      ...
      "not be able to be loaded from settings" when {
          ...
          assertDecodingFailure(
            ...
            assertion = errors => {
              ...
              errors.head should be(
                RulesLevelCreationError(
                  Message(s"No user definitions was defined. Rule `$simpleSyntaxName` requires them.")
                )
              )
            }
          )
        ...
        "username in user definitions is empty" in {
          assertDecodingFailure(
            ...
            assertion = errors => {
              ...
              errors.head should be(
                DefinitionsLevelCreationError(
                  Message("Error for field 'username': Non empty list of user ID patterns are required")
                )
              )
            }
          )
        ...
        "username is used in two definitions (when the duplication check is implicitly enabled)" in {
          val basicAuthenticationRules =
            List("auth_key", "auth_key_sha1", "auth_key_sha256", "auth_key_sha512", "auth_key_pbkdf2", "auth_key_unix")
          ...
          testCases.foreach { case (authRule1, authRule2) =>
            assertDecodingFailure(
              yaml = s"""
                        ...
                        |""".stripMargin,
              assertion = errors => {
                errors shouldEqual NonEmptyList.of(
                  DefinitionsLevelCreationError(
                    Message(
      ...
      "auth key rule inside user definition is unparsable" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(DefinitionsLevelCreationError(MalformedValue.fromString("""auth_key:
                                                                                            |  key: "cartman:pass"
                                                                                            |""".stripMargin)))
          }
        )
      ...
      "user definition doesn't allow to use unknown rules" in {
        assertDecodingFailure(
          ...
          assertion = errors => {
            errors should have size 1
            errors.head should be(
              DefinitionsLevelCreationError(Message("Unknown rule 'unknown_field' in users definitions section"))
            )
          }
        )
      }
        ...
        assertDecodingFailure(
          ...
          assertion = errors => {
            ...
            errors.head should be(
              DefinitionsLevelCreationError(
                Message(
                  "Users [cartman] are allowed to be authenticated by rule [auth_key], but it's used in a context of user patterns [a*]. It seems that this is not what you expect."
                )
              )
            )
          }
            ...
            assertDecodingSuccess(
              ...
              assertion = rule => {
                ...
                val expectedGroupsLogic = GroupsLogic.Combined(
                  GroupsLogic.AnyOf(
                    GroupIds(UniqueNonEmptyList.of(GroupId("group3"), GroupIdPattern.fromNes(nes("group4*"))))
                  ),
                  GroupsLogic.NotAnyOf(
                    GroupIds(UniqueNonEmptyList.of(GroupId("group5"), GroupIdPattern.fromNes(nes("group6*"))))
                  ),
                )
                resolvedGroupsLogic should contain(expectedGroupsLogic)

                val permittedGroupsLogic =
                  rule.settings.permittedGroupsLogic.asInstanceOf[RuntimeResolvableGroupsLogic.Combined]
                permittedGroupsLogic.positive
                  .asInstanceOf[RuntimeResolvableGroupsLogic.Simple[GroupsLogic.AnyOf]]
                  .groupIds should
                  be(
                    UniqueNonEmptyList.of(
                      AlreadyResolved(GroupId("group3").nel),
                      AlreadyResolved(GroupIdPattern.fromNes("group4*").nel)
                    )
                  )
                permittedGroupsLogic.negative
                  .asInstanceOf[RuntimeResolvableGroupsLogic.Simple[GroupsLogic.NotAnyOf]]
                  .groupIds should
                  be(
                    UniqueNonEmptyList.of(
                      AlreadyResolved(GroupId("group5").nel),
                      AlreadyResolved(GroupIdPattern.fromNes("group6*").nel)
                    )
                  )
        ...
        "advanced groups mapping with structured groups is used" in {
          assertDecodingSuccess(
            ...
            assertion = rule => {
              ...
              permittedGroupsLogic.negative
                .asInstanceOf[RuntimeResolvableGroupsLogic.Simple[GroupsLogic.NotAllOf]]
                .groupIds should
                be(
                  UniqueNonEmptyList.of(
                    AlreadyResolved(GroupId("group5").nel),
  ...
  private def currentUserMetadataRequestBlockContextFrom(
      update: BlockMetadata => BlockMetadata = identity,
