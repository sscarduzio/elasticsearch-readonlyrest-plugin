  ...
  "A StringOps method toNonEmptyStringsTuple" should {
    "be able to create two non-empty string tuple from string" when {
      "string contains one colon somewhere in the middle" in {
        "example:test".toNonEmptyStringsTuple should be(
          Right(NonEmptyString.unsafeFrom("example"), NonEmptyString.unsafeFrom("test"))
        )
      }
      "string contains two colons" in {
        "example:test:test".toNonEmptyStringsTuple should be(
          Right(NonEmptyString.unsafeFrom("example"), NonEmptyString.unsafeFrom("test:test"))
        )
      }
      "there are two colons at the end of string" in {
        "example::".toNonEmptyStringsTuple should be(
          Right(NonEmptyString.unsafeFrom("example"), NonEmptyString.unsafeFrom(":"))
        )
      }
    }
