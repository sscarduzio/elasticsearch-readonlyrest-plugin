...
class UsersRuleTests extends WordSpec with MockFactory {
  ...
  private def assertMatchRule(configuredIds: NonEmptySet[RuntimeSingleResolvableVariable[User.Id]], loggedUser: Option[LoggedUser]) =
    assertRule(configuredIds, loggedUser, isMatched = true)

  private def assertNotMatchRule(configuredIds: NonEmptySet[RuntimeSingleResolvableVariable[User.Id]], loggedUser: Option[LoggedUser]) =
    assertRule(configuredIds, loggedUser, isMatched = false)

  private def assertRule(configuredIds: NonEmptySet[RuntimeSingleResolvableVariable[User.Id]], loggedUser: Option[LoggedUser], isMatched: Boolean) = {
