...
class GroupsLogicExecutorTests extends AnyWordSpec with Matchers {
  ...
  "GroupsLogic AND" should {
    "handle properly filtering of available groups from user groups" when {
      "permitted groups are all wildcarded" in {
        ...
        val result = and.availableGroupsFrom(
          UniqueNonEmptyList.of(
            group("abc"),
            group("bca"),
            group("cab"),
      ...
      "permitted groups are full groups names" in {
        val and = GroupsLogic.AllOf(
          GroupIds(
            UniqueNonEmptyList.of(
              GroupIdLike.from("abc"),
              GroupIdLike.from("cba")
            )
          )
        )
        val result = and.availableGroupsFrom(
  ...
  "GroupsLogic NOT_ALL_OF" should {
    "handle properly filtering of available groups from user groups" when {
      "forbidden groups are all wildcarded (1)" in {
        val notAllOf = GroupsLogic.NotAllOf(
          GroupIds(
            UniqueNonEmptyList.of(
              GroupIdLike.from("a*"),
              GroupIdLike.from("b*")
        ...
        val result = notAllOf.availableGroupsFrom(
          UniqueNonEmptyList.of(
            group("abc"),
            group("cab"),
            group("cba")
  ...
  "GroupsLogic NOT_ANY_OF" should {
    "handle properly filtering of available groups from user groups" when {
      "forbidden groups are all wildcarded (1)" in {
        val notAnyOf = GroupsLogic.NotAnyOf(
          GroupIds(
            UniqueNonEmptyList.of(
              GroupIdLike.from("a*"),
              GroupIdLike.from("b*")
        ...
        val result = notAnyOf.availableGroupsFrom(
          UniqueNonEmptyList.of(
            group("abc"),
            group("cab"),
            group("cba")
      ...
      "OR rule with wildcards and NOT_ANY_OF rule with full names (1)" in {
        ...
        val notAnyOf = GroupsLogic.NotAnyOf(
          GroupIds(
            UniqueNonEmptyList.of(
              GroupIdLike.from("abc"),
              GroupIdLike.from("bac")
            )
          )
        )
        val combined = GroupsLogic.Combined(or, notAnyOf)
        val result = combined.availableGroupsFrom(
          UniqueNonEmptyList.of(
            group("aaa"),
            group("bbb"),
            group("caa")
      ...
      "OR rule with full names and NOT_ANY_OF rule with wildcards (1)" in {
        val or = GroupsLogic.AnyOf(
          GroupIds(
            UniqueNonEmptyList.of(
              GroupIdLike.from("abc"),
              GroupIdLike.from("bca"),
              GroupIdLike.from("cab")
            )
          )
        )
        val notAnyOf = GroupsLogic.NotAnyOf(
