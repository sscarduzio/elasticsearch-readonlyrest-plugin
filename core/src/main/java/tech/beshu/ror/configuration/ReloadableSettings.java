...
public abstract class ReloadableSettings {
  ...
  private final AtomicReference<BasicSettings> rorSettings = new AtomicReference<>();
  private final SettingsManager settingsManager;
  private final LoggerShim logger;
  private CompletableFuture<Void> clientReadyFuture;
  private WeakHashMap<Consumer<BasicSettings>, Boolean> onSettingsUpdateListeners = new WeakHashMap<>();

  public ReloadableSettings(SettingsManager settingsManager) throws IOException {

    this.rorSettings.set(new BasicSettings(new RawSettings(settingsManager.getSettings())));
  ...
  public void addListener(Consumer<BasicSettings> onSettingsUpdate) {
    ...
    return clientReadyFuture
      .thenCompose((x) ->
                     CompletableFuture.supplyAsync(() -> {
                                                     ...
                                                     BasicSettings ror = new BasicSettings(raw);
                       ...
                       .exceptionally(th -> {
                         if (th instanceof SettingsMalformedException) {
                           ...
                           logger.debug("[CLUSTERWIDE SETTINGS] not sure if settings not found or conf error.. " + th.getClass().getName() + " " + th.getMessage());
                         }
