...
import tech.beshu.ror.acl.AclLoggingDecorator._
...
class AclLoggingDecorator(underlying: Acl, serializationTool: Option[SerializationTool])
  extends Acl with StrictLogging {
    ...
    if (!shouldSkipLog(state, blockVerbosity)) {
      logger.info {
        s"""${stateColor(state)}
           |${state.show}
           | by $reason
           | req=${resultShow(requestContext, blockContext, history)}
           | ${Constants.ANSI_RESET}""".stripMargin
      }
  ...
  private def resultShow(r: RequestContext, blockContext: Option[BlockContext], history: Vector[Block.History]) = {
    ...
    s"""{
       ...
       | KDX: ${blockContext.flatMap(_.kibanaIndex).getOrElse("")},
       | ACT: ${r.action.show},
