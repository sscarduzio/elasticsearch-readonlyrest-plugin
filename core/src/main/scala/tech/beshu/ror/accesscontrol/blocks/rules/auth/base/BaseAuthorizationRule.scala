...
import tech.beshu.ror.accesscontrol.blocks.rules.auth.base.impersonation.{
  ...
  SimpleAuthorizationImpersonationSupport
}
...
private[auth] trait BaseAuthorizationRule extends AuthorizationRule with SimpleAuthorizationImpersonationSupport {

  protected def groupsLogic: GroupsLogic

  protected def userIdCaseSensitivity: CaseSensitivity

  protected def userGroups[B <: BlockContext](blockContext: B, user: LoggedUser)(
  ...
  private def authorizeImpersonatedUser[B <: BlockContext: BlockContextUpdater](
      blockContext: B,
  ...
  private def doAuthorizeLoggedUser[B <: BlockContext: BlockContextUpdater](blockContext: B, user: LoggedUser)(
      implicit requestId: RequestId
  ): Task[Decision[B]] = {
    authorizeLoggedUser(
  ...
  private def authorizeLoggedUser[B <: BlockContext: BlockContextUpdater](
      blockContext: B,
      user: LoggedUser,
      fetchUserGroups: (B, LoggedUser) => Task[UniqueList[Group]]
  ): Task[Decision[B]] = {
  ...
  private def checkIfCurrentGroupIsEligible(blockContext: BlockContext, userGroups: UniqueNonEmptyList[Group]) = {
    EitherT.cond[Task](
