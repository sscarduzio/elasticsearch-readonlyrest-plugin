...
trait AccessControlList {
  ...
  def handleRegularRequest[B <: BlockContext: BlockContextUpdater](
...
object AccessControlList {
  ...
  object RegularRequestResult {
    final case class Allowed[B <: BlockContext](matchedBlockContext: B) extends RegularRequestResult[B]
    final case class Forbidden[B <: BlockContext](matchedBlockContext: B) extends RegularRequestResult[B]
    ...
    final case class IndexNotFound[B <: BlockContext](allowedClusters: Set[ClusterName.Full])
        extends RegularRequestResult[B]
    final case class AliasNotFound[B <: BlockContext]() extends RegularRequestResult[B]
    final case class TemplateNotFound[B <: BlockContext]() extends RegularRequestResult[B]
    final case class Failed[B <: BlockContext](ex: Throwable) extends RegularRequestResult[B]
    final case class PassedThrough[B <: BlockContext]() extends RegularRequestResult[B]
  }
  ...
  object ForbiddenCause {
    ...
    def from(cause: Denied.Cause): ForbiddenCause = {
      cause match {
        ...
        case _: Denied.Cause.OtherFailure | _: Denied.Cause.AuthenticationFailure |
