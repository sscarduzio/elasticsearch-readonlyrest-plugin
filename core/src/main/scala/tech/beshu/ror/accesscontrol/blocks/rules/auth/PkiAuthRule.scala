...
final class PkiAuthRule(val authentication: PkiAuthenticationRule, val authorization: PkiAuthorizationRule)
    extends BaseComposedAuthenticationAndAuthorizationRule(authentication, authorization) {

  override val name: Rule.Name = PkiAuthRule.Name.name
...
object PkiAuthRule {
  ...
  implicit case object Name extends RuleName[PkiAuthRule] {
    override val name = Rule.Name("pki_auth")
  }
