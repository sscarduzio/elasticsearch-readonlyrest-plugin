...
object AclValidator {
  ...
  def validate(
      blockDecodingResultsOpt: Option[List[BlockDecodingResult]],
      userDefs: Definitions[UserDef]
  ): ValidatedNel[String, NonEmptyList[BlockDecodingResult]] = {
  ...
  private def validateAclPresentAndNonEmpty(
      blockDecodingResultsOpt: Option[List[BlockDecodingResult]]
  ): ValidatedNel[String, NonEmptyList[BlockDecodingResult]] =
  ...
  private def validateThatUsersConfigSectionIsUsedWhenPresent(
      blocks: List[Block],
      userDefs: Definitions[UserDef]
  ): ValidatedNel[String, Unit] = {
    val thereAreUserDefinitions = userDefs.items.nonEmpty
    lazy val thereIsGroupsRule = blocks
      .flatMap(_.rules.toList)
      .collect { case rule: BaseGroupsRule[_] =>
        rule
      }
      .nonEmpty
