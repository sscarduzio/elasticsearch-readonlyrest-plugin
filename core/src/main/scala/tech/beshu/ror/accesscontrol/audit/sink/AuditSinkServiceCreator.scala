...
sealed trait AuditSinkServiceCreator {
  ...
  protected final def withConnectivityCheck[A](cluster: AuditCluster,
                                               httpClientsFactory: HttpClientsFactory,
                                               create: Task[A]): Task[Either[AuditSinkServiceCreator.InitializationError, A]] =
...
object AuditSinkServiceCreator extends RequestIdAwareLogging {
  ...
  private def withConnectivityCheck[A](cluster: AuditCluster,
                                       httpClientsFactory: HttpClientsFactory,
                                       create: Task[A]): Task[Either[InitializationError, A]] = {
    cluster match {
      ...
      case remote: RemoteAuditCluster =>
        new AuditRemoteClusterConnectivityCheck(httpClientsFactory)
          ...
          .flatMap {
            case Right(()) =>
              create.map(Right(_))
            case Left(error: AuditRemoteClusterConnectivityCheck.Error.ConnectivityError) if remote.ignoreClusterConnectivityProblems =>
              Task.delay(noRequestIdLogger.warn(s"Audit cluster connectivity check failed, but 'ignore_es_connectivity_problems: true' is set, so auditing will proceed: ${error.message}"))
                .flatMap(_ => create.map(Right(_)))
            case Left(error: AuditRemoteClusterConnectivityCheck.Error.ConnectivityError) =>
              Task.pure(Left(InitializationError(
...
trait IndexBasedAuditSinkServiceCreator extends AuditSinkServiceCreator {
  ...
  final def createIndexService(cluster: AuditCluster,
                               httpClientsFactory: HttpClientsFactory): Task[Either[AuditSinkServiceCreator.InitializationError, IndexBasedAuditSinkService]] =
...
trait DataStreamAndIndexBasedAuditSinkServiceCreator
  ...
  final def createDataStreamService(cluster: AuditCluster,
                                    httpClientsFactory: HttpClientsFactory): Task[Either[AuditSinkServiceCreator.InitializationError, DataStreamBasedAuditSinkService]] =
