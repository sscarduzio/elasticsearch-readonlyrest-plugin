...
class UnboundidLdapDefaultGroupSearchAuthorizationServiceWithServerSideGroupsFiltering private (
  ...
  private def groupsFrom(mode: DefaultGroupSearch, user: LdapUser, filteringGroupIds: Set[GroupIdLike])(
      implicit requestId: RequestId
  ): Task[UniqueList[Group]] = {
    connectionPool
      ...
      .flatMap {
        ...
        case Left(errorResult) =>
          logger.error(
            s"LDAP getting user groups returned error: [code=${errorResult.getResultCode.toString.show}, cause=${errorResult.getResultString.show}]"
          )
          Task.raiseError(LdapUnexpectedResult(errorResult.getResultCode, errorResult.getResultString))
      }
  }

  private def searchUserGroupsLdapRequest(
    ...
    logger.debug(
      s"LDAP search [base DN: ${baseDn.show}, scope: ${scope.getName().show}, search filter: ${searchFilter.show}, attributes: ${groupAttributes.show}]"
    )
    new SearchRequest(listener, baseDn, scope, searchFilter, groupAttributes.toSeq*)
  }

  private def searchUserGroupsLdapFilerFrom(
      mode: DefaultGroupSearch,
      user: LdapUser,
      filteringGroupIds: Set[GroupIdLike]
  ) = {
    ...
    val serverSideGroupsFiltering = filteringGroupIds.toList match {
      ...
      case manyGroups      =>
        manyGroups.map(filterPartGroupId(mode.groupAttribute, _)).map(g => s"($g)").sorted.mkString("(|", "", ")")
    }
...
object UnboundidLdapDefaultGroupSearchAuthorizationServiceWithServerSideGroupsFiltering {
  ...
  def create(
      ...
      implicit clock: Clock
  ): Task[Either[ConnectionError, LdapAuthorizationService.WithGroupsFiltering]] = {
