...
class IndicesRule(override val settings: Settings,
  ...
  private def processAliasRequest(blockContext: AliasRequestBlockContext): Task[Decision[AliasRequestBlockContext]] = {
    if (matchAll) {
      ...
      val allowedIndices = allowedIndicesFor(blockContext)
      for {
        indicesResult <- processIndices(blockContext.requestContext, allowedIndices, blockContext.indices, kibanaIndexFrom(blockContext))
        aliasesResult <- processIndices(blockContext.requestContext, allowedIndices, blockContext.aliases, kibanaIndexFrom(blockContext))
      } yield {
  ...
  private def getAllowedClusterNames(requestContext: RequestContext,
                                     allowedIndices: AllowedClusterIndices): Set[ClusterName.Full] = {
