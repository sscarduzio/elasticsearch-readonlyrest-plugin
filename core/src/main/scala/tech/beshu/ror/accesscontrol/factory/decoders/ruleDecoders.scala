...
object ruleDecoders {
  ...
  def ruleDecoderBy(
    ...
    val optionalRuleDecoder = name match {
      case ruleName @ (Rule.Name("groups") | AllOfGroupsRule.DeprecatedSimpleSyntaxNameV1.name |
          AllOfGroupsRule.DeprecatedSimpleSyntaxNameV2.name | AllOfGroupsRule.SimpleSyntaxName.name |
          AnyOfGroupsRule.DeprecatedSimpleSyntaxNameV1.name | AnyOfGroupsRule.DeprecatedSimpleSyntaxNameV2.name |
          AnyOfGroupsRule.DeprecatedSimpleSyntaxNameV3.name | AnyOfGroupsRule.SimpleSyntaxName.name |
          NotAllOfGroupsRule.SimpleSyntaxName.name | NotAnyOfGroupsRule.SimpleSyntaxName.name) =>
        implicit val ruleNameForDecoder: RuleName[BaseGroupsRule[GroupsLogic]] =
          new RuleName[BaseGroupsRule[GroupsLogic]] {
      ...
      case IndicesRule.Name.name              =>
        Some(new IndicesRuleDecoders(variableCreator, systemContext.uniqueIdentifierGenerator))
      case KibanaUserDataRule.Name.name =>
        Some(new KibanaUserDataRuleDecoder(globalSettings.settingsIndex, variableCreator)(systemContext.jsCompiler))
      ...
      case SessionMaxIdleRule.Name.name      =>
        Some(new SessionMaxIdleRuleDecoder(globalSettings)(systemContext.clock, systemContext.uuidProvider))
  ...
  def usersDefinitionsAllowedRulesDecoderBy(
      name: Rule.Name,
    ...
    val optionalRuleDecoder = name match {
      case ExternalAuthorizationRule.Name.name =>
        Some(
          new ExternalAuthorizationRuleDecoder(
      ...
      case LdapAuthorizationRule.Name.name =>
        Some(
          new LdapAuthorizationRuleDecoder(
      ...
      case RorKbnAuthenticationRule.Name.name =>
        Some(
          new RorKbnRulesDecoders.AuthenticationRuleDecoder(
            rorKbnDefinitions.items,
            rorKbnDefinitions.items,
            globalSettings
          )
        )
  ...
  def authenticationRuleDecoderBy(
      name: Rule.Name,
      authenticationServiceDefinitions: Definitions[ExternalAuthenticationService],
      authProxyDefinitions: Definitions[ProxyAuth],
      ldapServiceDefinitions: Definitions[LdapService],
      ...
      globalSettings: GlobalSettings,
      esEnv: EsEnv
  ): Option[RuleDecoder[AuthenticationRule]] = {
    val optionalRuleDecoder = name match {
      ...
      case ExternalAuthenticationRule.Name.name =>
        Some(
          new ExternalAuthenticationRuleDecoder(
            authenticationServiceDefinitions,
            impersonatorsDefinitions,
            mocksProvider,
            globalSettings
      ...
      case LdapAuthenticationRule.Name.name =>
        Some(
          new LdapAuthenticationRuleDecoder(
            ldapServiceDefinitions,
            impersonatorsDefinitions,
            mocksProvider,
            globalSettings
  ...
  def withUserIdParamsCheck[R <: Rule](
      decoder: RuleDecoder[R],
      userIdPatterns: UserIdPatterns,
      globalSettings: GlobalSettings,
      errorCreator: Message => DecodingFailure
  ): Decoder[RuleDecoder.Result[R]] = {
  ...
  private def checkUsersEligibility(
