...
class AccessControlListLoggingDecorator(val underlying: AccessControlList,
  ...
  override def handleMetadataRequest(requestContext: UserMetadataRequestContext.Aux[UserMetadataRequestBlockContext]): Task[WithHistory[UserMetadataRequestResult, UserMetadataRequestBlockContext]] = {
      ...
      case Allowed(_, userMetadata, _) =>
        userMetadata match {
          case UserMetadata.WithoutGroups(_, _, _, metadataOrigin) =>
            Some(metadataOrigin.block.audit)
          case UserMetadata.WithGroups(groupsMetadata) =>
            val auditsFromGroupMetadataBlocks = groupsMetadata.values.map(_.metadataOrigin.block.audit)
            Some {
