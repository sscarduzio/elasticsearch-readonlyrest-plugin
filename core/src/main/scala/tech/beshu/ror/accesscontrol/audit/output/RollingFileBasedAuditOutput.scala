...
import tech.beshu.ror.accesscontrol.audit.AuditingTool.AuditSettings.AuditOutputConfig.Config.RollingFileBasedOutput.FileAppenderConfig
import tech.beshu.ror.accesscontrol.domain.{AuditOutputName, RorAuditLoggerName}
import tech.beshu.ror.utils.RequestIdAwareLogging

private[audit] final class RollingFileBasedAuditOutput private (
...
object RollingFileBasedAuditOutput extends RequestIdAwareLogging {
    ...
    directoryError(config.filePath) match {
      ...
      case None      =>
        buildAndRegisterAppender(loggerName, config)
          .map(appender => Right(new RollingFileBasedAuditOutput(outputName, serializer, loggerName, appender)))
          .onErrorHandle { ex =>
