...
class UnboundidLdapGroupsFromUserEntryAuthorizationService private (
  ...
  private def groupsFrom(mode: GroupsFromUserEntry, user: LdapUser)(
      implicit requestId: RequestId
  ): Task[UniqueList[Group]] = {
    connectionPool
      ...
      .flatMap {
        ...
        case Left(errorResult) if errorResult.getResultCode == ResultCode.NO_SUCH_OBJECT && !user.confirmed =>
          logger.error(s"LDAP getting user groups returned error [${errorResult.show}]")
  ...
  private def searchUserGroupsLdapRequest(
    ...
    logger.debug(
      s"LDAP search [base DN: ${baseDn.show}, scope: ${scope.show}, search filter: ${searchFilter.show}, attributes: ${attribute.show}]"
    )
