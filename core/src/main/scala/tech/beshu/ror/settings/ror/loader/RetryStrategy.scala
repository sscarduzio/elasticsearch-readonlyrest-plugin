...
trait RetryStrategy {
  ...
  def withRetry[ERROR: Show, RESULT](operation: Task[Either[ERROR, RESULT]], operationDescription: String)(
      implicit requestId: RequestId
  ): Task[Either[ERROR, RESULT]]

  def withRetryT[ERROR: Show, RESULT](operation: EitherT[Task, ERROR, RESULT], operationDescription: String)(
      implicit requestId: RequestId
  ): EitherT[Task, ERROR, RESULT] =
    EitherT(withRetry(operation.value, operationDescription))

}

class ConfigurableRetryStrategy(config: LoadingRetryStrategySettings) extends RetryStrategy with RequestIdAwareLogging {

  override def withRetry[ERROR: Show, RESULT](operation: Task[Either[ERROR, RESULT]], operationDescription: String)(
      implicit requestId: RequestId
  ): Task[Either[ERROR, RESULT]] =
    attemptWithRetry(operation, currentAttempt = 1, config.attemptsCount.value.value, operationDescription)

  private def attemptWithRetry[ERROR: Show, A](
      operation: Task[Either[ERROR, A]],
      currentAttempt: Int,
      maxAttempts: Int,
    ...
    for {
      ...
      finalResult <- result match {
        ...
        case Left(error) if shouldRetry(currentAttempt, maxAttempts) =>
          logger.debug(
            s"$operationDescription - retry attempt $currentAttempt/$maxAttempts failed. Retrying in ${config.attemptsInterval.show}..."
          )
