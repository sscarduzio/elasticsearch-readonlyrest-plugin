...
import tech.beshu.ror.acl.{Acl, EnabledAclStaticContext}
...
object RorEngineFactory {
  ...
  final class __old_Engine(val acl: Acl, val context: EnabledAclStaticContext, httpClientsFactory: AsyncHttpClientsFactory) {
    def shutdown(): Unit = {
