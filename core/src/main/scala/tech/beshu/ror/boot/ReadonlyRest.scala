...
class ReadonlyRest(
  ...
  private def createEngine(
    ...
    EitherT(createAuditingTool(coreCreationResult.auditSetup, engineResources.httpClientsFactory))
      .map { auditingTool =>
  ...
  private def createAuditingTool[O <: AuditOutput](auditSetup: AuditSetup[O], httpClientsFactory: HttpClientsFactory)(
