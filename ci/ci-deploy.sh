...
# The plugin and elasticsearch versions are taken from the build name, which comes from gradle.build (or pom.xml)
#
# Ultimately, I'm just going to commit changes to the build.gradle (or pom.xml) and this thing tags and uploads where necessary.

CONF_FILE="conf.json"
BUCKET="readonlyrest-data"
DISTRIBUTION_PATH="target/"
