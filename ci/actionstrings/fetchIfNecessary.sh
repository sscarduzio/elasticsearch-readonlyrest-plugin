...
if test -f "$FILENAME"; then
  ci_log "$FILENAME exists."
  exit 0
fi

ci_log "processing ES action extractions for: $FILENAME"
...
if ! ci/actionstrings/fetch.sh "v$ES_VERSION" > "$TMP_FILE"; then
  ci_log "extraction failed for ES $ES_VERSION"
  exit 1
fi
if [ ! -s "$TMP_FILE" ]; then
  ci_log "no action strings found for ES $ES_VERSION (is the tag v$ES_VERSION published?)"
  exit 1
fi
