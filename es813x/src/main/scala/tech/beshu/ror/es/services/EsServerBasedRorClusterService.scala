...
class EsServerBasedRorClusterService(nodeName: String,
  ...
  private def determineAllSnapshots()(implicit id: RequestContext.Id): Map[RepositoryName.Full, Task[Set[Snapshot]]] = {
  ...
  private def resolveAllRemoteDataStreams(remoteClusterName: ClusterName.Full,
                                         ...
                                         (implicit id: RequestContext.Id): Task[List[FullRemoteDataStreamWithAliases]] = {
  ...
  private def getRemoteClusterClient(remoteClusterService: RemoteClusterService,
    ...
    Try(remoteClusterService.getRemoteClusterClient(remoteClusterName.value.value, scheduler))
  }
  ...
  private def allSnapshotsFrom(repository: RepositoryName.Full)
                              (implicit id: RequestContext.Id): Task[Set[Snapshot]] = {
