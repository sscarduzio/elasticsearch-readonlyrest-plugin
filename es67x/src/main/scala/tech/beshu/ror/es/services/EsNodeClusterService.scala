...
class EsNodeClusterService(nodeName: String,
    ...
    remoteClusterServiceSupplier.get() match {
      case Some(remoteClusterService) =>
        remoteClusterService
          .getRegisteredRemoteClusterNames.asScala
  ...
  override def indexTemplates: Set[Template.IndexTemplate] = Set.empty

  override def componentTemplates: Set[Template.ComponentTemplate] = Set.empty
          ...
          snapshotsService
            ...
            .flatMap { snapshotId =>
              snapshotFullNameFrom(snapshotId).map {
                name => Snapshot(name, snapshotsService.getRepositoriesService.getSnapshotIndices(repository, snapshotId))
              }
            }
        }
      case None =>
