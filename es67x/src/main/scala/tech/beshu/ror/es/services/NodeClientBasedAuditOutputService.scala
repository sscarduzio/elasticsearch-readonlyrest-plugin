...
final class NodeClientBasedAuditOutputService(client: NodeClient)
  ...
  private val bulkProcessor =
    BulkProcessor
      .builder(client, new AuditOutputBulkProcessorListener) // deprecated since es 6.8.5
  ...
  private def submitDocument(
    ...
    val request = new IndexRequest(indexName.name.value, "ror_audit_evt", documentId)
