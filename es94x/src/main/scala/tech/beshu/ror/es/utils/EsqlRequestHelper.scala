...
object EsqlRequestHelper {
  ...
  private final class EsqlParser(
    ...
    private def planOf(statement: Any): Any = on(statement).call("plan").get[Any]()
    ...
    private val reviewedPlanLeaves: Map[String, PlanLeafReview.Verdict] = Map(
      ...
      "PlaceholderRelation" -> PlanLeafReview.Verdict.NotAnIndexSource,
      "ScalarFunction" -> PlanLeafReview.Verdict.NotAnIndexSource,
      "UnresolvedExternalRelation" -> PlanLeafReview.Verdict.UnsupportedIndexSource,
      "ExternalRelation" -> PlanLeafReview.Verdict.UnsupportedIndexSource,
      "ParameterizedQuery" -> PlanLeafReview.Verdict.UnsupportedIndexSource
    )
