...
object EsqlRequestHelper {
  ...
  private final class EsqlParser(
    ...
    private def indexTablesFrom(statement: Any): Either[EsqlClassificationError, List[EsqlIndexTable]] = {
      ...
      indexTablesFromPreAnalysis(preAnalysis)
    }

    private def indexTablesFromPreAnalysis(preAnalysis: Any): Either[EsqlClassificationError, List[EsqlIndexTable]] = {
      for {
    ...
    private def sourceCommandIndexLists(preAnalysis: Any): List[String] = {
      indexListsOf(on(preAnalysis).get[java.util.Map[Any, Any]]("indexes").keySet().asScala.toList)
    }

    private def lookupJoinIndexLists(preAnalysis: Any): List[String] = {
      indexListsOf(on(preAnalysis).get[java.util.List[Any]]("lookupIndices").asScala.toList)
    }

    private def indexListsOf(indexPatterns: List[Any]): List[String] = {
      indexPatterns.map(indexPattern => on(indexPattern).call("indexPattern").get[String]())
    }
