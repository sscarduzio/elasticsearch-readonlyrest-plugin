...
import org.elasticsearch.common.xcontent.{NamedXContentRegistry, XContentBuilder, XContentType}
...
trait ResponseFieldsFiltering {
  ...
  private def filterBytesRestResponse(response: BytesRestResponse, fieldsRestrictions: ResponseFieldsRestrictions): BytesRestResponse = {
    ...
    val parser = xContent.createParser(NamedXContentRegistry.EMPTY, response.content().streamInput())
