...
|---|---|---|
| `ROR_ES_SLIM_MODULES` | `true` | Strip the ES modules no suite uses from the test image. This applies to both docker images (the official Elasticsearch one and the Ubuntu one with ES from apt); the native Windows process keeps every module. Set `false` to run against a stock ES image, e.g. when adding a suite for a feature whose module the slim image drops. |
