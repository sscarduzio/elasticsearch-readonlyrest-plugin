...
import tech.beshu.ror.es.providers.EsAuditSink;
import tech.beshu.ror.es.providers.EsIndexJsonContentProvider;
import tech.beshu.ror.es.requests.ForbiddenResponse;
import tech.beshu.ror.es.requests.RequestInfo;
import tech.beshu.ror.es.requests.regular.RegularResponseActionListener;
import tech.beshu.ror.es.requests.RorNotReadyResponse;
...
public class IndexLevelActionFilter implements ActionFilter {
  ...
  private <Request extends ActionRequest, Response extends ActionResponse> Function1<Either<Throwable, WithHistory<RegularRequestResult>>, BoxedUnit> handleAclResult(
      Engine engine,
      ActionListener<Response> listener,
      Request request,
    ...
    return result -> {
      try (ThreadContext.StoredContext ctx = threadPool.getThreadContext().stashContext()) {
        if (result.isRight()) {
          AccessControlActionHandler handler = createAclActionHandler(engine.context(), requestInfo, request, requestContext,
