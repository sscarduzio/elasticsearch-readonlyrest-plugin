...
class RestoreSnapshotEsRequestContext(actionRequest: RestoreSnapshotRequest,
  ...
  private def snapshotFrom(blockContext: SnapshotRequestBlockContext) = {
    ...
    snapshots match {
      ...
      case snapshot :: rest =>
        if (rest.nonEmpty) {
          logger.warn(s"[${blockContext.requestContext.id.show}] Filtered result contains more than one snapshot. First was taken. Whole set of repositories [${snapshots.mkString(",")}]")
        }
        Right(snapshot)
    }
  }
